...
import org.shop.entity.dto.UserLocalDTO;
...
public class GuestRefreshTokenInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        String key = RedisConstant.LOGIN_USER_KEY_GUEST + token;     //基于TOKEN获取redis中的用户
        Map<Object, Object> userMap = stringRedisTemplate.opsForHash().entries(key);
        log.info("操作用户信息: " + userMap);

        if (userMap.isEmpty()) throw new AccountNotFoundException(MessageConstant.ACCOUNT_NOT_FOUND);
        ...
        stringRedisTemplate.expire(key, RedisConstant.LOGIN_USER_TTL_GUEST, TimeUnit.MINUTES);      // 一次用户操作, 能够刷新token有效期
